...
impl BufferView {
    pub fn new(buffer: &Rc<RefCell<Buffer>>) -> BufferView {
        BufferView {
            point: 0..0,
            buffer: buffer.clone(),
        }
    ...
    pub fn move_point_forward_char(&mut self) {
        if self.point.end < self.buffer.borrow().rope.len_chars() {
            self.point.end += 1;
    ...
    pub fn move_point_end_of_line(&mut self) {
        let line_idx = self.buffer.borrow().rope.char_to_line(self.point.end);
        let idx = if line_idx == 0 {
            self.buffer.borrow().rope.len_chars()
        } else {
            self.buffer.borrow().rope.line_to_char(line_idx + 1) - 1
        };
        self.point.start = idx;
        self.point.end = idx;
    }
    pub fn move_point_start_of_line(&mut self) {
        let line_idx = self.buffer.borrow().rope.char_to_line(self.point.start);
    ...
    pub fn goto_char(&mut self, char_idx: usize) {
        let idx = min(char_idx, self.buffer.borrow().rope.len_chars());
        self.point.start = idx;
        self.point.end = idx;
    }
    ...
    pub fn goto_line(&mut self, line_idx: usize) {
        let idx = self
            ...
            .borrow()
            .rope
            .line_to_char(min(self.buffer.borrow().rope.len_lines(), line_idx));
        self.point.start = idx;
        self.point.end = idx;
    }
    pub fn goto_end_of_buffer(&mut self) {
        let len = { self.buffer.borrow().rope.len_chars() };
        self.goto_char(len);
    }
    ...
    pub fn insert_at_point(&mut self, text: &str) {
        self.buffer.borrow_mut().rope.insert(self.point.start, text);
        let off = Rope::from(text).len_chars();
        self.point.start += off;
        self.point.end = self.point.start;
        self.buffer.borrow_mut().is_modified = true;
...
mod tests {
    ...
    fn new_buffer() {
        ...
        assert_eq!(buf.borrow().rope.len_bytes(), 0);
